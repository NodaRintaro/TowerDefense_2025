...
// エディタウィンドウ
public class GettingDataCreateWindow : EditorWindow
{
    ...
    // 新規データホルダー作成用
    private DataType _gettingDataType = DataType.None;
    ...
    private string _saveDataName = null;

    [MenuItem("Tools/GettingDebugDataCreateEditorWindow")]
    public static void ShowWindow()
    {
        GettingDataCreateWindow window = GetWindow<GettingDataCreateWindow>("GettingDataHolder Editor");
    ...
    private void DrawDataHolderCreationSection()
    {
        ...
        _gettingDataType = (DataType)EditorGUILayout.EnumPopup(_gettingDataType, GUILayout.Width(120));

        if (GUILayout.Button("Create", GUILayout.Width(100)))
        {
            _dataHolders.Add(new GettingDataHolder(_gettingDataType));
    ...
    private void DrawDataHolderList()
    {
        ...
        for (int i = 0; i < _dataHolders.Count; i++)
        {
            ...
            string displayName = $"[{_dataHolders[i].DataType}] Data {i + 1}";
            ...
            if (GUILayout.Button("X", GUILayout.Width(25)))
            {
                if (EditorUtility.DisplayDialog("Delete Data Holder",
                    $"Are you sure you want to delete 'Data {i + 1}'?", "Yes", "No"))
                {
    ...
    {
        switch (_gettingDataType)
        {
            case DataType.CharacterData:
                _saveDataName = SaveDataPathHolder.Save_Json_CharacterGettingDataPath;
                break;
            case DataType.SupportCard:
                _saveDataName = SaveDataPathHolder.Save_Json_SupportCardGettingDataPath;
                break;
        }
